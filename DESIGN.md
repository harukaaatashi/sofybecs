...
- **機能とテーマの違い**: 機能タグ（HOME/記録/チャット…）は**画面単位**、テーマ（旧アプリ移行/同期・消失/予測のズレ…）は**症状単位**。同じ口コミが両方に属する。テーマは `src/lib.ts` の `THEMES` に正規表現で定義した**固定リスト**で、実データを見て決めている（n-gram による自動抽出も試したが、形態素解析器なしでは断片が4割混じって使えず、解析器はバンドル制約に反するため不採用）。どのテーマにも当たらない口コミは無理に分類せずどこにも数えない
